...
-- Pre-calculate values used in loops to avoid repeated calculations
local screenWidth = _screen.w
local screenHeight = _screen.h
local colWidth = (screenWidth / numCols) - margin.w
local rowHeight = (screenHeight - (margin.h * (numRows - 2))) / (numRows - 2)

-- Initialize col and row tables outside of the loop to avoid redundancy
local col = {
	how_many = numCols,
	w = colWidth,
}
...
for player in ivalues(PlayerNumber) do
	-- Initialize the OptionsWheel table for this player only once
	OptionsWheel[player] = {}
	
	-- Use a single setmetatable call with a pre-built table
	local optionRowsForPlayer = {}
	for i = 1, #OptionRows do
		optionRowsForPlayer[i] = {}
	end
	OptionsWheel[player] = setmetatable(optionRowsForPlayer, {__index = function(t, k) return setmetatable(t[k], sick_wheel_mt) end, disable_wrapping = true})
end
...
local PruneSongsFromGroup = function(group)
    ...
    local index = 1
    local longVerSongSeconds = PREFSMAN:GetPreference("LongVerSongSeconds")
    local casualMaxMeter = ThemePrefs.Get("CasualMaxMeter")

    -- Iterate through songs in the group once, applying all filters in a single pass
    for i, song in ipairs(SONGMAN:GetSongsInGroup(group)) do
        if song:HasStepsType(steps_type)
        and song:GetLastSecond() < longVerSongSeconds
        and UNLOCKMAN:IsSongLocked(song) == 0 then
            for _, steps in ipairs(song:GetStepsByStepsType(steps_type)) do
                if steps:GetMeter() <= casualMaxMeter then
                    songs[#songs + 1] = song
                    -- Check if this is the current song after adding it to the list
                    if current_song == song then index = #songs end
                    break -- Found a valid steps, no need to check further steps for this song
                end
            end
        end
    end
