...
af[#af+1] = Def.Quad{
    ...
    InitCommand=function(self)
        self:zoomto(_screen.w, _screen.h/(row.how_many-2)):diffuse(0,0,0,1):cropbottom(1)
    end,
...
af[#af+1] = Def.Quad{
    InitCommand=function(self)
        self:zoomto(_screen.w, 1):diffuse(1,1,1,0)
        :xy(_screen.cx, _screen.cy+30 + _screen.h/(row.how_many-2)*-0.5)
        :faderight(10):rainbow()
    end,
    OnCommand=function(self)
        self:sleep(0.3):diffusealpha(0.75):queuecommand("FadeMe")
    end,
    FadeMeCommand=function(self)
        self:accelerate(1.5):faderight(0):accelerate(1.5):fadeleft(10)
        :sleep(0):diffusealpha(0):fadeleft(0):sleep(1.5):faderight(10)
        :diffusealpha(0.75):queuecommand("FadeMe")
    end,
...
af[#af+1] = Def.ActorFrame{
	...
	LoadActor("./img/arrow_glow.png")..{
		InitCommand=function(self) self:zoom(0.25):diffuseshift():effectcolor1(1,1,1,0):effectcolor2(1,1,1,1) end,
		OnCommand=function(self)
			local parent = self:GetParent()
			if parent:GetName() == "RightArrow" then
				self:x(_screen.cx-50)
			elseif parent:GetName() == "LeftArrow" then
				self:x(-_screen.cx+50):rotationz(180)
			end
		end,
	},
	...
	Def.ActorFrame{
		...
		Def.BitmapText{
			...
			InitCommand=function(self) self:zoom(0.65):diffuse(Color.White):y(14):horizalign(left) end,
	 		CurrentSongChangedMessageCommand=function(self, params)
				if params.song then
		 			self:settext( THEME:GetString("ScreenSelectMusic", "Length") .. ": " .. SecondsToMMSS(params.song:GetLastSecond()):gsub("^0*","") )
				end
	 		end,
		},
		-- genre
