...
local choices = {
	{
		...
		pads = {
			{color=GetHexColor(SL.DefaultColor+1), offset=0}
		}
	},
	{
		name="versus",
		x=_screen.cx,
		pads = {
			{color=GetHexColor(SL.DefaultColor),   offset=-WideScale(42,51)},
			{color=GetHexColor(SL.DefaultColor+3), offset= WideScale(42,51)}
		}
	},
	{
		name="double",
		x=_screen.cx+_screen.w/4,
		pads = {
			{color=GetHexColor(SL.DefaultColor+2), offset=-WideScale(42,51)},
			{color=GetHexColor(SL.DefaultColor+2), offset= WideScale(42,51)}
		}
	},
}
...
if current_game=="dance" and ThemePrefs.Get("AllowDanceSolo") then
	...
	choices[4] = { name="solo", pads={ {color=GetHexColor(SL.DefaultColor+1), offset=0}}, x=_screen.w-_screen.w/8 }
...
local t = Def.ActorFrame{
	...
	OnCommand=function(self)
		-- enforce the game environment here
		EnforceGameEnvironment()
