...
getFavoritesPath = function(player)
	local path = PROFILEMAN:GetProfileDir(ProfileSlot[PlayerNumber:Reverse()[player] + 1]) .. "favorites.txt"
	return path
end

addOrRemoveFavorite = function(player)
	local profileName = PROFILEMAN:GetPlayerName(player) == "" and ToEnumShortString(player)
		or PROFILEMAN:GetPlayerName(player)
...
generateFavoritesForMusicWheel = function()
	for pn in ivalues(GAMESTATE:GetEnabledPlayers()) do
		...
		if PROFILEMAN:IsPersistentProfile(pn) then
			...
			local profileName = PROFILEMAN:GetPlayerName(pn) == "" and ToEnumShortString(pn)
				or PROFILEMAN:GetPlayerName(pn)
				...
				if favs:len() > 2 then
					...
					for line in favs:gmatch("[^\r\n]+") do
						...
						if line:find("^---") then
							...
							SL[ToEnumShortString(pn)].Favorites[#SL[ToEnumShortString(pn)].Favorites + 1] =
