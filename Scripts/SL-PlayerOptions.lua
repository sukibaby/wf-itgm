...
local Overrides = {
	...
	-------------------------------------------------------------------------
	SpeedModType = {
		--Choices = { "Multiplier (x-mod)", "Constant (c-mod)", "Max (m-mod)" },
		--Values = { "X", "C", "M" },
		Choices = function()
				local speedmod_descs = { "Multiplier (x-mod)", "Constant (c-mod)", "Max (m-mod)" }
				
				-- Add average to the list if it's supported (Outfox, currently)
				-- kind of a hacky check since we have no pn, is there anything better?
				local poptions = GAMESTATE:GetPlayerState(0):GetPlayerOptions("ModsLevel_Preferred")
				local has_average_support = poptions.AMod ~= nil
				if has_average_support then table.insert(speedmod_descs, "Average (a-mod)") end
				
				return speedmod_descs
			end,
			Values = function()
				local speedmod_shorthands = { "X", "C", "M" }
				
				local poptions = GAMESTATE:GetPlayerState(0):GetPlayerOptions("ModsLevel_Preferred")
				local has_average_support = poptions.AMod ~= nil
				if has_average_support then table.insert(speedmod_shorthands, "A") end
				
				return speedmod_shorthands
			end,
		ExportOnChange = true,
	...
	JudgmentGraphic = {
		...
		SaveSelections = function(self, list, pn)
			...
			-- append "_ITG" to the name if player is using ITG settings
			local name = mods.JudgmentGraphic
	...
	EarlyLate = {
		ExportOnChange = true,
		Choices = { "Text", "Tick", "Side Tick", "Error Bar", "None" },
		Values = { "Enabled", "Simple", "SideTick", "Advanced", "Disabled" }
	},
	EarlyLateColor = {
		Choices = { "None", "Early/Late", "Judgment Colors" },
		Values = { "Disabled", "EarlyLate", "Judgment" }
	},
	-------------------------------------------------------------------------
	BackgroundFilter = {
	...
	ScreenDarken = {
		...
		LoadSelections = function(self, list, pn)
			local mods = SL[ToEnumShortString(pn)].ActiveModifiers
			local darken = ("%g"):format(SL[ToEnumShortString(pn)].ActiveModifiers.ScreenDarken)
			local i = FindInTable(darken, self.Choices) or 0.95
			list[i] = true			
			return list
		end,
		SaveSelections = function(self, list, pn)
			...
			for i=1,#self.Choices do
				if list[i] then
					mods.ScreenDarken = tonumber( self.Choices[i] )
				end
			end
		end,
	},
	...
	Hide = {
		...
		Values = { "Targets", "SongBG", "Combo", "Lifebar", "Score", "Danger" },
		LoadSelections = function(self, list, pn)
			local mods = SL[ToEnumShortString(pn)].ActiveModifiers
	...
	DataVisualizations = {
		OneChoiceForAllPlayers = true,
		Values = function()
			local choices = { "None", "Step Statistics" }
	...
	StepStatsExtras = {
		SelectType = "SelectMultiple",		
		Values = { "StepInfo", "Groovestats", "TimeElapsed" },
		LoadSelections = function(self, list, pn)
			local mods = SL[ToEnumShortString(pn)].ActiveModifiers
			list[1] = mods.StepInfo  	or false
			list[2] = mods.Groovestats 	or false
			list[3] = mods.TimeElapsed 	or false
			return list
		end,
		SaveSelections = function(self, list, pn)
			local mods, playeroptions = GetModsAndPlayerOptions(pn)
			mods.StepInfo  		= list[1]
			mods.Groovestats 	= list[2]
			mods.TimeElapsed	= list[3]			
		end,
	},
	GIF = {
		Choices = { "None", "catJAM", "CrabPls", "Snoop Dogg", "Dancing Duck", "Sonic", "Nyan Cat" },
		Values = { "None", "catJAM", "CrabPls", "Snoop", "Dancing Duck", "Sonic", "Nyan Cat" }
	},
	--GIFEffects = {
	--	SelectType = "SelectMultiple",
	--	Values = { "Responsive", "DVD Logo", "Random Movement" },
	--	LoadSelections = function(self, list, pn)
	--		local mods = SL[ToEnumShortString(pn)].ActiveModifiers
	--		list[1] = mods.GIFResponsive	or false
	--		list[2] = mods.GIFDVD			or false
	--		list[3] = mods.GIFRandom		or false
	--		return list
	--	end,
	--	SaveSelections = function(self, list, pn)
	--		local mods, playeroptions = GetModsAndPlayerOptions(pn)
	--		mods.GIFResponsive	= list[1]
	--		mods.GIFDVD			= list[2]
	--		mods.GIFRandom		= list[3]
	--	end,
	--},	
	ComboEffects = {
		SelectType = "SelectMultiple",
	...
	ComboStretch = {
		SelectType = "SelectMultiple",
		Values = { "Grow", "Horizontal", "Vertical", "Random" },
		LoadSelections = function(self, list, pn)
			local mods = SL[ToEnumShortString(pn)].ActiveModifiers
			list[1] = mods.ComboStretchGrow			or false
			list[2] = mods.ComboStretchHorizontal	or false
			list[3] = mods.ComboStretchVertical		or false
			list[4] = mods.ComboStretchRandom		or false
			return list
		end,
		SaveSelections = function(self, list, pn)
			local mods, playeroptions = GetModsAndPlayerOptions(pn)
			mods.ComboStretchGrow		= list[1]
			mods.ComboStretchHorizontal	= list[2]
			mods.ComboStretchVertical	= list[3]
			mods.ComboStretchRandom 	= list[4]
		end,
	},
	ComboRotation = {
		Choices = { "None", "Combo", "Random" },
		Values = { "None", "Combo", "Random" }
		--Choices = { "None", "Combo", "Random", "BPM" },
		--Values = { "None", "Combo", "Random", "BPM" }
	},
	ComboRainbow = {
		Choices = { "None", "Always", "100", "250", "500", "1000" },
		Values = { "None", "Always", "100", "250", "500", "1000" }
	},
	Nice = {
		SelectType = "SelectMultiple",
		Values = { "Combo", "Judgements" },
		LoadSelections = function(self, list, pn)
			local mods = SL[ToEnumShortString(pn)].ActiveModifiers
			list[1] = mods.NiceSoundCombo		or false
			list[2] = mods.NiceSoundJudgements	or false
			return list
		end,
		SaveSelections = function(self, list, pn)
			local mods, playeroptions = GetModsAndPlayerOptions(pn)
			mods.NiceSoundCombo			= list[1]
	...
	TargetScore = {
		...
		LoadSelections = function(self, list, pn)
			local i = tonumber(SL[ToEnumShortString(pn)].ActiveModifiers.TargetScore) or 3
			list[i] = true
			return list
		end,
		SaveSelections = function(self, list, pn)
	...
	SubtractiveScoring = {
		...
		Values = { "Off", "Original", "Full", "Predicted", "Score Pace" },
	},
	-------------------------------------------------------------------------
	SubtractiveEnvironment = {
		--Choices = { "Off", "Original", "Full", "Predicted" },
		Values = { "Default", "Waterfall", "ITG", "EX" },
	},
	-------------------------------------------------------------------------
	-- Not used currently
	PacemakerText = {
		Values = { "Off", "On", "96", "98", "99", "Machine best", "Personal best", "Rivals" },
	},
	-------------------------------------------------------------------------
	MeasureCounter = {
	...
	-------------------------------------------------------------------------
	LifeMeterType = {
		Values = { "Standard", "Vertical", "Surround" },
	},
	-------------------------------------------------------------------------
	ScreenAfterPlayerOptions = {
		Values = function()
			local choices = { "Gameplay", "Select Music", "Options2", "Options4"  }
	...
	ScreenAfterPlayerOptions2 = {
		Values = function()
			local choices = { "Gameplay", "Select Music", "Options1", "Options4"  }
			if SL.Global.MenuTimer.ScreenSelectMusic < 1 then table.remove(choices, 2) end
			return choices
		end,
		OneChoiceForAllPlayers = true,
		SaveSelections = function(self, list, pn)
			...
			if SL.Global.MenuTimer.ScreenSelectMusic > 1 then
				...
				if list[3] then SL.Global.ScreenAfter.PlayerOptions2 = "ScreenPlayerOptions1" end
				if list[4] then SL.Global.ScreenAfter.PlayerOptions2 = "ScreenPlayerOptions4" end
			end
		end
	},
	...
	-------------------------------------------------------------------------
	-------------------------------------------------------------------------
	-- Options relating to ITG simulation
	ITGOptions = {
		SelectType = "SelectOne",
		Choices = { "Standard", "ITG" },
		LayoutType = "ShowAllInRow",
		OneChoiceForAllPlayers = false,
		ExportOnChange = false,
		LoadSelections = function(self, list, pn)
			local p = ToEnumShortString(pn)
			list[2] = SL[p].ActiveModifiers.SimulateITGEnv
			list[1] = (not list[2])
			return list
		end,
		SaveSelections = function(self, list, pn)
			local p = ToEnumShortString(pn)
			SL[p].ActiveModifiers.SimulateITGEnv = list[2]
		end
	},
	-- FA+
	-- 12.5ms option no longer exists
	FAPlus = {
		Choices = { "Disabled", "Insane (10ms)", "15ms (for ITG)" },
		Values = { 0, 0.010, 0.015 },
		ExportOnChange = false,
		SaveSelections = function(self, list, pn)
			local mods = SL[ToEnumShortString(pn)].ActiveModifiers
			if (not mods.SimulateITGEnv) and list[4] then
				mods.FAPlus = 0
				return
			end
			for i, v in ipairs(self.Values) do
	...
	EarlyLateThreshold = {
		Choices = { "FA+", "Top Judgment", "None" },
		ExportOnChange = true
	},
	-- Option for Error Window
	ExpErrorWindow = {
		Choices = { "Enabled", "Disabled", "Extended" },
		ExportOnChange = true,
		LayoutType = "ShowAllInRow",
		OneChoiceForAllPlayers = true,
		LoadSelections = function(self, list, pn)
			...
			for i,v in ipairs(c) do
				if math.abs(PREFSMAN:GetPreference("TimingWindowSecondsW5") - WF.GetErrorWindowVal(v)) < 0.00001 then
					list[i] = true
					return list
				end
			end
			
			-- if we reach here, just set it to normal
			WF.SetErrorWindow(self.Choices[1])
			list[1] = true
			return list
		end,
		SaveSelections = function(self, list, pn)
			local c = self.Choices
			local s
			for i,v in ipairs(c) do if list[i] then s = v break end end
			if s then WF.SetErrorWindow(s) end
		end
	},
	-- Options to show EX Scoring, originally implemented for ITL 2022
	EXScoring = {
		SelectType = "SelectOne",
		Choices = { "Off", "On" },
		Values = { false, true },
		...
		LoadSelections = function(self, list, pn)
			local p = ToEnumShortString(pn)
			list[2] = SL[p].ActiveModifiers.EXScoring
			list[1] = (not list[2])
			return list
		end,
		SaveSelections = function(self, list, pn)
			local p = ToEnumShortString(pn)
			SL[p].ActiveModifiers.EXScoring = list[2]
		end
	},
	-- LifeBar options
	PreferredLifeBar = {
	...
	-- Some kind of silly logic going on here in order to implement these settings as "player options" while they're
	-- actually set from the customize profile menu. May do some refactoring later, but MAYBE NOT LOL
	ProfileCardInfo = {
		OneChoiceForAllPlayers = true,
		ExportOnChange = true,
		Choices = { "Songs Played", "Mastery-SC Counts", "AC-FC Counts", "HCL-ECL Counts", "ITG Star Grade Counts" },
		Values = { "SongsPlayed", "FCTiers", "FCTiersNoPerfect", "LifeBarClears", "TopGradesITG" },
		LoadSelections = function(self, list, pn)
			if SCREENMAN:GetTopScreen():GetName() == "ScreenCustomProfile" then
				for i,v in ipairs(self.Values) do
					if WF.CustomProfileOptions.ProfileCardInfo == v then list[i] = true end
				end
			end
			return list
		end,
		SaveSelections = function(self, list, pn)
			if SCREENMAN:GetTopScreen():GetName() == "ScreenCustomProfile" then
				for i,v in ipairs(self.Values) do
					if list[i] then WF.CustomProfileOptions.ProfileCardInfo = v end
				end
			end
		end
	},
	PreferredSecondPane = {
		OneChoiceForAllPlayers = true,
		ExportOnChange = true,
		Choices = { "Default", "Per Panel Breakdown", "Timing Distribution" },
		Values = { "General", "PerPanel", "Timing" },
		LoadSelections = function(self, list, pn)
			if SCREENMAN:GetTopScreen():GetName() == "ScreenCustomProfile" then
				for i,v in ipairs(self.Values) do
					if WF.CustomProfileOptions.PreferredSecondPane == v then list[i] = true end
				end
			else
				local mods = SL[ToEnumShortString(pn)].ActiveModifiers
				list[FindInTable(mods.PreferredSecondPane, self.Values) or 1] = true
			end
			return list
		end,
		SaveSelections = function(self, list, pn)
			if SCREENMAN:GetTopScreen():GetName() == "ScreenCustomProfile" then
				for i,v in ipairs(self.Values) do
					if list[i] then WF.CustomProfileOptions.PreferredSecondPane = v end
				end
			else
				local mods = SL[ToEnumShortString(pn)].ActiveModifiers
				mods.PreferredSecondPane = self.Values[FindInTable(true, list) or 1]
			end
		end
	},
	PreferredGraph = {
		OneChoiceForAllPlayers = true,
		ExportOnChange = true,
		Choices = { "Life Graph", "Timing Scatterplot" },
		Values = { "Life", "Scatterplot" },
		LoadSelections = function(self, list, pn)
			if SCREENMAN:GetTopScreen():GetName() == "ScreenCustomProfile" then
				for i,v in ipairs(self.Values) do
					if WF.CustomProfileOptions.PreferredGraph == v then list[i] = true end
				end
			else
				local mods = SL[ToEnumShortString(pn)].ActiveModifiers
				list[FindInTable(mods.PreferredGraph, self.Values) or 1] = true
			end
			return list
		end,
		SaveSelections = function(self, list, pn)
			if SCREENMAN:GetTopScreen():GetName() == "ScreenCustomProfile" then
				for i,v in ipairs(self.Values) do
					if list[i] then WF.CustomProfileOptions.PreferredGraph = v end
				end
			else
				local mods = SL[ToEnumShortString(pn)].ActiveModifiers
				mods.PreferredGraph = self.Values[FindInTable(true, list) or 1]
			end
		end
	},
	PreferredSecondGraph = {
		OneChoiceForAllPlayers = true,
		ExportOnChange = true,
		Choices = { "Life Graph", "Timing Scatterplot" },
		Values = { "Life", "Scatterplot" },
		LoadSelections = function(self, list, pn)
			if SCREENMAN:GetTopScreen():GetName() == "ScreenCustomProfile" then
				for i,v in ipairs(self.Values) do
					if WF.CustomProfileOptions.PreferredSecondGraph == v then list[i] = true end
				end
			else
				local mods = SL[ToEnumShortString(pn)].ActiveModifiers
				list[FindInTable(mods.PreferredSecondGraph, self.Values) or 1] = true
			end
			return list
		end,
		SaveSelections = function(self, list, pn)
			if SCREENMAN:GetTopScreen():GetName() == "ScreenCustomProfile" then
				for i,v in ipairs(self.Values) do
					if list[i] then WF.CustomProfileOptions.PreferredSecondGraph = v end
				end
			else
				local mods = SL[ToEnumShortString(pn)].ActiveModifiers
				mods.PreferredSecondGraph = self.Values[FindInTable(true, list) or 1]
			end
		end
	},
	PreferredGameEnv = {
		...
		ExportOnChange = true,
		Choices = { "Standard", "ITG" },
		Values = { "Waterfall", "ITG" },
		LoadSelections = function(self, list, pn)
			if SCREENMAN:GetTopScreen():GetName() == "ScreenCustomProfile" then
				for i,v in ipairs(self.Values) do
					if WF.CustomProfileOptions.PreferredGameEnv == v then list[i] = true end
				end
			end
			return list
		end,
		SaveSelections = function(self, list, pn)
			if SCREENMAN:GetTopScreen():GetName() == "ScreenCustomProfile" then
				for i,v in ipairs(self.Values) do
					if list[i] then WF.CustomProfileOptions.PreferredGameEnv = v end
				end
			end
		end
	},
	PreferredFaultWindow = {
		OneChoiceForAllPlayers = true,
		ExportOnChange = true,
		Choices = { "Enabled", "Disabled", "Extended" },
		Values = { 1, 2, 3 },
		LoadSelections = function(self, list, pn)
			if SCREENMAN:GetTopScreen():GetName() == "ScreenCustomProfile" then
				for i,v in ipairs(self.Values) do
					if WF.CustomProfileOptions.PreferredFaultWindow == v then list[i] = true end
				end
			end
			return list
		end,
		SaveSelections = function(self, list, pn)
			if SCREENMAN:GetTopScreen():GetName() == "ScreenCustomProfile" then
				for i,v in ipairs(self.Values) do
					if list[i] then WF.CustomProfileOptions.PreferredFaultWindow = v end
				end
			end
		end
	},
	GSOverride = {
		...
		LoadSelections = function(self, list, pn)
			if SCREENMAN:GetTopScreen():GetName() == "ScreenCustomProfile" then
				for i,v in ipairs(self.Values) do
					if WF.CustomProfileOptions.GSOverride == v then list[i] = true end
				end
			else
				local mods = SL[ToEnumShortString(pn)].ActiveModifiers
				list[FindInTable(mods.GSOverride, self.Values) or 1] = true
			end
			return list
		end,
		SaveSelections = function(self, list, pn)
			if SCREENMAN:GetTopScreen():GetName() == "ScreenCustomProfile" then
				for i,v in ipairs(self.Values) do
					if list[i] then WF.CustomProfileOptions.GSOverride = v end
				end
			else
				local mods = SL[ToEnumShortString(pn)].ActiveModifiers
...
function ApplyMods(player)
	...
	-- assign game environment to load into based on preference from profile
	local pn = ToEnumShortString(player)
	SL[pn].ActiveModifiers.SimulateITGEnv = (SL[pn].ActiveModifiers.PreferredGameEnv == "ITG")
	
	if player == GAMESTATE:GetMasterPlayerNumber() then
		-- Preferred fault window set in the customise profile menu	
		-- So tech players dont have to set fault window to disabled every time	
		local preferredFaults = SL[ToEnumShortString(player)].ActiveModifiers.PreferredFaultWindow	
		local faults
		
		if preferredFaults == 1 then faults = "Enabled"
		elseif preferredFaults == 2 then faults = "Disabled"
		elseif preferredFaults == 3 then faults = "Extended"
		else 
			preferredFaults = 1
			faults = "Enabled"
		end
		
		WF.SelectedErrorWindowSetting = preferredFaults
		
		--WF.SelectedErrorWindowSetting = 1 -- {1 = Enabled, 2 = Disabled, 3 = Extended}
		WF.SetErrorWindow(faults)			
	end
		
	-- Set preferred fault window setting
	SL[pn].ActiveModifiers.ExpErrorWindow = SL[pn].ActiveModifiers.PreferredFaultWindow
