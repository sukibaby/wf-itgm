...
local combo_bmt = LoadFont("_Combo Fonts/Wendy/Wendy")..{
	...
	ColorCommand=function(self, params)
		...
		if mods.ComboEffectsWild then
			...
			self:rotationz(math.random(1, 360)):zoom(math.random() * combo / math.min(100, combo)):x(math.random(nfw * -1,nfw)):y(math.random(nfw * -1,nfw))
			local rand = math.random()
			if params.Misses then self:rainbowscroll(false):stopeffect():diffuse( Color.Red )
			elseif rand > 0.8 then
				self:rainbowscroll(true)
			elseif rand > 0.6 then
				self:rainbowscroll(false):stopeffect():diffuse( colors.FullComboW1[2] )
			elseif rand > 0.4 then
				self:rainbowscroll(false):stopeffect():diffuse( colors.FullComboW2[2] )
			elseif rand > 0.2 then
				self:rainbowscroll(false):stopeffect():diffuse( colors.FullComboW3[2] )
			else
				self:rainbowscroll(false):stopeffect():diffuse( Color.White )
			end
		end
	end,
	JudgmentMessageCommand = function(self, params)
		...
		if (not useitg) and params.TapNoteScore and (params.TapNoteScore == "TapNoteScore_HitMine") then
			...
			if mods.ComboStretchGrow then self:zoom(0.75) end
			if mods.ComboStretchRandom  then self:zoom(0.75) end
			if mods.ComboStretchHorizontal  then self:zoomx(0.75) end
			if mods.ComboStretchVertical  then self:zoomy(0.75) end
		end
	end,
	...
	ButtonPressMessageCommand = function(self, params)
		if params.Player == player then
			if mods.ComboEffectsResponsive then
				if params.Button == "Left" then self:addx(-1.5)
				elseif params.Button == "Right" then self:addx(1.5)
				elseif params.Button == "Up" then self:addy(-1.5)
				elseif params.Button == "Down" then self:addy(1.5)
				end
			end
			if mods.ComboEffectsResponsiveInverse then					
				if params.Button == "Left" then self:addx(1.5)
				elseif params.Button == "Right" then self:addx(-1.5)
				elseif params.Button == "Up" then self:addy(1.5)
				elseif params.Button == "Down" then self:addy(-1.5)
